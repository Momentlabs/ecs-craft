...
func TestProxyTaskDefSelect(t *testing.T) {
  ...
  for _, v := range testPositiveVals {
    // proxyTaskDefArg = v[0]
    assert.Equal(t, v[1], getProxyTaskDef(v[0]), "Expecting proxyTaskDef: %s to yield: %s", v[0], v[1])
  }
  ...
  for _, v := range testNegativeVals {
    // proxyTaskDefArg = v[0]
    assert.NotEqual(t, v[1], getProxyTaskDef(v[0]), "Expecting proxyTaskDef: %s to yied: %s", v[0], v[1])
  }
 }